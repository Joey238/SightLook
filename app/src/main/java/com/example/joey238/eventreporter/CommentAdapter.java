...
public class CommentAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int type = getItemViewType(position);//we have two types: event or comment, 对应的地方给正确的layout.

        if (rowView == null) {
            ViewHolder viewHolder = new ViewHolder();
            switch (type) {
                case TYPE_EVENT:  //widgets inilization
            ...
            rowView.setTag(viewHolder);//所有子view都放在了reviewholder里，建立起与view的链接
        }
        ...
        if (type == TYPE_EVENT) {
            ...
            final Comment comment = commentList.get(position - 1);//-1: 总有event在上面，所以要-1；
